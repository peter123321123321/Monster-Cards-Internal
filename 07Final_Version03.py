    ...
    while True:
        ...
        if change == "Name":
            ...
            edit_stat = eg.integerbox(f"The original stat was {cards[monster_name][change]}"
                                      f"what would you like the new stat to be",
                                      "Stat change",
                                      lowerbound=1, upperbound=25)
...
def find_show():
    ...
    choice = eg.buttonbox("Would you like to view a MONSTER or show all MONSTERS",
                          "Find or Show MONSTERS",
    ...
    if choice == "Show MONSTERS":
        ...
        while find != "Exit":
            ...
            # then replace the old one with new one
            if change == "Name":
                new_name = eg.enterbox("What would you like the new MONSTERS name to be",
                                       "Change MONSTER Name")
                # Check if user presses cancel
                if new_name is None:
                    break
                ...
                edit_stat = eg.integerbox(f"The original stat was {cards[find][change]} "
                                          f"what would you like the new stat to be",
                                          "Stat change", lowerbound=1, upperbound=25)
                # Check if user pressed cancel
                if edit_stat is None:
                    break
                # Add new stat to dictionary
...
# List of MONSTERS
monsters = ["Stoneling", "Vexscream", "Dawnmirage", "Blazegolem", "Websnake",
